...
import (
	...
	"github.com/a1994sc/provider-nexus/apis"
	"github.com/a1994sc/provider-nexus/apis/v1alpha1"
	"github.com/a1994sc/provider-nexus/config"
	"github.com/a1994sc/provider-nexus/internal/clients"
	"github.com/a1994sc/provider-nexus/internal/controller"
	"github.com/a1994sc/provider-nexus/internal/features"
...
func main() {
	var (
		app = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Nexus").
			DefaultEnvars()
		...
		terraformVersion = app.Flag("terraform-version", "Terraform version.").
					Required().
					Envar("TERRAFORM_VERSION").
					String()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-nexus"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-nexus",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add Nexus APIs to scheme")
	...
	if *enableExternalSecretStores {
		...
		kingpin.FatalIfError(
			resource.Ignore(
				...
				mgr.GetClient().Create(context.Background(), &v1alpha1.StoreConfig{
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup Nexus controllers")
